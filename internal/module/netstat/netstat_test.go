...
func TestNetstat(t *testing.T) {
	...
	t.Run("TCP Over IPv4", func(t *testing.T) {
		...
		for _, conn := range conns {
			const format = "%-21s  %-21s  %-11s  %-6d  %s\n"
			state := GetTCPConnState(conn.State)
			fmt.Printf(format, conn.LocalAddr(), conn.RemoteAddr(), state, conn.PID, conn.Process)
		}
	...
	t.Run("UDP Over IPv4", func(t *testing.T) {
		...
		require.NoError(t, err)
		fmt.Println("Local Address          Remote Address         PID     Process")
		for _, conn := range conns {
			const format = "%-21s  %-21s  %-6d  %s\n"
			fmt.Printf(format, conn.LocalAddr(), conn.RemoteAddr(), conn.PID, conn.Process)
		}
		testsuite.IsDestroyed(t, &conns)
	})
...
func TestTCP4Conn_Clone(t *testing.T) {
	conn := &TCP4Conn{
		LocalIP:    net.IP{0x01, 0x02, 0x03, 0x04},
		LocalPort:  0x1127,
		RemoteIP:   net.IP{0x05, 0x06, 0x07, 0x08},
...
func TestTCP6Conn_Clone(t *testing.T) {
	conn := &TCP6Conn{
		LocalIP: net.IP{
			0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
			0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
...
func TestUDP4Conn_Clone(t *testing.T) {
	conn := &UDP4Conn{
		LocalIP:   net.IP{0x01, 0x02, 0x03, 0x04},
		LocalPort: 0x1127,
	}
...
func TestUDP6Conn_Clone(t *testing.T) {
	conn := &UDP6Conn{
		LocalIP: net.IP{
			0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
			0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
